...
function ActionRejectClaim() {
    ...
    const rejectClaimMutation = useMutation<ResponseQuery<Claim>, Error, ExternalClaimRejectProps>(async (props) => await externalActionRejectClaim(props),
        {
            onSuccess: (data) => {
    ...
    >
        ...
        {
            isLoading ? <CircularProgress color="inherit" /> : isEditing ? < RemarkActionClaimCard actionType="claim" referenceNo={actionRef} onSubmitRejectClaim={onSubmitRejectClaim} />
                : isSuccess ? <SuccessRejectCard actionType="claim" referenceNo={rejectClaimMutation.data ? rejectClaimMutation.data.data[0].transactionId : "-"}  /> : <ErrorClaimActionCard />
...
export default ActionRejectClaim;